...
class EcoliGraph(object):
    ...
    def dnaG(self, input_tensor, noise_mean=0, noise_stdev=0):
        input_tensor = input_tensor[..., :1]  # [yheI]
        new_tensor = torch.tensor([[0.1170 + (0.1955) * i[0]] for i in input_tensor])
    ...
    def hupB(self, input_tensor, noise_mean=0, noise_stdev=0):
        ...
        new_tensor = torch.tensor([[-0.1821 + (-0.2984) * i[0]] for i in input_tensor])
    ...
    def mopB(self, input_tensor, noise_mean=0, noise_stdev=0):
        input_tensor = input_tensor[..., :2]  # [lacZ]
    ...
    def pspA(self, input_tensor, noise_mean=0, noise_stdev=0):
        input_tensor = input_tensor[..., :3]  # [cspG, yedE]
        new_tensor = torch.tensor([[-0.1901 + (0.1079) * i[0] + (-0.7456) * i[1]] for i in input_tensor])
    ...
    def __init__(self, num_observations = 1000, num_objective_points = None):
        ...
        obs_data_ygcE = self.ygcE(torch.cat([obs_data_b1191,obs_data_sucA], dim=1), noise_mean = 0 )
        obs_data_asnA = self.asnA(torch.cat([obs_data_ygcE], dim=1), noise_mean = 0 )
        obs_data_icdA = self.icdA(torch.cat([obs_data_asnA,obs_data_ygcE], dim=1), noise_mean = 0 )
        obs_data_aceB = self.aceB(torch.cat([obs_data_icdA], dim=1), noise_mean = 0 )
        obs_data_atpD = self.atpD(torch.cat([obs_data_sucA,obs_data_ygcE], dim=1), noise_mean = 0 )
        ...
        obs_data_lacZ = self.lacZ(torch.cat([obs_data_asnA,obs_data_lacA,obs_data_lacY], dim=1), noise_mean = 0 )
        ...
        obs_data_dnaG = self.dnaG(torch.cat([obs_data_yheI], dim=1), noise_mean = 0 )
        ...
        obs_data_hupB = self.hupB(torch.cat([obs_data_cspA], dim=1), noise_mean = 0 )
        ...
        obs_data_pspA = self.pspA(torch.cat([obs_data_cspG,obs_data_yedE], dim=1), noise_mean = 0 )
