...
class AspirinModel(ConditionalStochasticModel):
    def draw_samples(self, parent_values: np.ndarray):
        age = torch.tensor(parent_values[:, 0], dtype=torch.float32)
        bmi = torch.tensor(parent_values[:, 1], dtype=torch.float32)
        logits = -8.0 + 0.1 * age + 0.03 * bmi
...
class CancerModel(ConditionalStochasticModel):
    def draw_samples(self, parent_values: np.ndarray):
        age = torch.tensor(parent_values[:, 0], dtype=torch.float32)
        bmi = torch.tensor(parent_values[:, 1], dtype=torch.float32)
        statin = torch.tensor(parent_values[:, 2], dtype=torch.float32)
        aspirin = torch.tensor(parent_values[:, 3], dtype=torch.float32)
        logits = 2.2 - 0.05 * age + 0.01 * bmi - 0.04 * statin + 0.02 * aspirin
        prob = torch.sigmoid(logits)
        return prob.unsqueeze(1).numpy()
...
class PSAModel(ConditionalStochasticModel):
    def draw_samples(self, parent_values: np.ndarray):
        ...
        statin = torch.tensor(parent_values[:, 2], dtype=torch.float32)
        aspirin = torch.tensor(parent_values[:, 3], dtype=torch.float32)
        cancer = torch.tensor(parent_values[:, 4], dtype=torch.float32)
        mean = 6.8 + 0.04 * age - 0.15 * bmi - 0.60 * statin + 0.55 * aspirin + 1.0 * cancer
...
class PSAGraph(object):
    ...
    # N(27.0 − 0.01 × age, 0.7)
    def bmi(self, input_tensor, noise_mean=0, noise_stdev=0):
    ...
    def __init__(self, num_observations = 1000, num_objective_points = None):
        ...
        self.graph.causal_model.set_causal_mechanism('BMI', BMIModel())
        ...
        ########################################################## Remove this for auto fitting ########################################
        self.true_graph.causal_model.set_causal_mechanism('AGE', AgeModel())
        self.true_graph.causal_model.set_causal_mechanism('BMI', BMIModel())
        self.true_graph.causal_model.set_causal_mechanism('ASPIRIN', AspirinModel())
        self.true_graph.causal_model.set_causal_mechanism('STATIN', StatinModel())
        self.true_graph.causal_model.set_causal_mechanism('CANCER', CancerModel())
        self.true_graph.causal_model.set_causal_mechanism('PSA', PSAModel())
        ################################################################################################################################

        # Generate observational data
        obs_data_age = self.age(num_observations)
        obs_data_bmi = self.bmi(obs_data_age, noise_mean=0, noise_stdev=0.2)
        obs_data_aspirin = self.aspirin(torch.cat([obs_data_age, obs_data_bmi], dim=1), noise_mean=0, noise_stdev=0.2)
        obs_data_statin = self.statin(torch.cat([obs_data_age, obs_data_bmi], dim=1), noise_mean=0, noise_stdev=0.2)
        obs_data_cancer = self.cancer(torch.cat([obs_data_age, obs_data_bmi, obs_data_statin, obs_data_aspirin], dim=1), noise_mean=0, noise_stdev=0.2)
        obs_data_psa = self.psa(torch.cat([obs_data_age, obs_data_bmi, obs_data_statin, obs_data_aspirin, obs_data_cancer], dim=1), noise_mean=0, noise_stdev=0.2)

        # Add to dataframe
        self.observational_samples = DataFrame()
        self.observational_samples['AGE'] = torch.flatten(obs_data_age).numpy()
        self.observational_samples['BMI'] = torch.flatten(obs_data_bmi).numpy()
        ...
        obs_data_age = self.age(num_objective_points)
        obs_data_bmi = self.bmi(obs_data_age)
        obs_data_aspirin = self.aspirin(torch.cat([obs_data_age, obs_data_bmi], dim=1))
        obs_data_statin = self.statin(torch.cat([obs_data_age, obs_data_bmi], dim=1))
        obs_data_cancer = self.cancer(torch.cat([obs_data_age, obs_data_bmi, obs_data_statin, obs_data_aspirin], dim=1))
        obs_data_psa = self.psa(torch.cat([obs_data_age, obs_data_bmi, obs_data_statin, obs_data_aspirin, obs_data_cancer], dim=1))

        # # Add to dataframe
        self.objective_samples = DataFrame()
        self.objective_samples['AGE'] = torch.flatten(obs_data_age).numpy()
        self.objective_samples['BMI'] = torch.flatten(obs_data_bmi).numpy()
        self.objective_samples['ASPIRIN'] = torch.flatten(obs_data_aspirin).numpy()
        self.objective_samples['STATIN'] = torch.flatten(obs_data_statin).numpy()
        self.objective_samples['CANCER'] = torch.flatten(obs_data_cancer).numpy()
        self.objective_samples['PSA'] = torch.flatten(obs_data_psa).numpy()
    ...
    def sample(self, n=1, do=None):
        ...
        parent_order = ["AGE", "BMI", "STATIN", "ASPIRIN", "CANCER"]
